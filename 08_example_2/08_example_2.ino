...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = _DIST_MIN;
    ledbright = 255;
    analogWrite(PIN_LED, ledbright);       // LED OFF
  } else if (distance > _DIST_MAX) {
    distance = _DIST_MAX;
    ledbright = 255;
    analogWrite(PIN_LED, ledbright);    // led off
  } else if (_DIST_MIN < distance < _DIST_MAX) {    // In desired Range
    ledbright = fabsf( 255 - 255*(distance - 100)/100 ); //* absolute value for top @ 200mm condition.
